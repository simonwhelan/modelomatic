...
//////////////////////////////////////////////////
// Note that the function only controls access to what can and cannot be optimised
// Hard settings of CPar::m_bOpt override the settings in this function
		...
		FOR(i,(int)m_vpProc.size()) {
			...
			if(m_vpProc[i]->NoSeq() >2) {
				FOR(j,m_vpProc[i]->Tree()->NoSeq()) {
					if(!m_vpProc[i]->Tree()->OptB(j)) { continue; }
...
bool CBaseModel::IsViable()	{
	...
	if(diff(Total,1.0)) {
		...
		cout << "\nNon-viable: "; FOR(i,(int)m_vpProc.size()) { cout << "\n\tProc["<<i<<"]: " << m_vpProc[i]->Name() << " = " << m_vpProc[i]->Prob() << "  "; }
		return false; }
...
//					cout << "\nInto GetBraDer, likelihood: " << lnL(true) << flush;
					...
					// Only store the branch parameters that are being optimised
					count = 0; FOR(j,m_vpProc[i]->Tree()->NoBra())	{ if(m_vpProc[i]->Tree()->OptB(j)) { temp[count] += m_vpAllOptPar[count]->grad(); count++; } }
			}	}
			if(ForceNumBra == true)	{
...
//				if(Tree()->NoSeq() > 2) { cout << "\n <<<<<<<<<<<<<<< DOING NUMERICAL DERIVATIVES: CurlnL = " << CurlnL << "; diff: "<< fabs(CurlnL-lnL()) <<" >>>>>>>>>>>>>>>>>>>>>>>>>>>>"; }
				...
				FOR(i,m_vpProc[0]->Tree()->NoBra()) {
					if(!m_vpProc[0]->Tree()->OptB(i)) { continue; }
					assert(m_vpAllOptPar[count]->IsBranch());
//					cout << "\nGetting branch derivative for ["<<count<<"]: " << *m_vpAllOptPar[count];
					temp[count] = m_vpAllOptPar[count]->grad(GetNumDerivative(m_vpAllOptPar[count]->OptimiserValue(),CurlnL));
//					cout << " ... have " << m_vpAllOptPar[count]->grad() << " = " << temp[count];
					count++;
				}
				// Error check here
				temp_lnL = lnL(true);
...
#if LIKELIHOOD_FUNC_DEBUG == 1
		cout << "\nProcess["<<ProcNum<<"]: Prob = " << m_vpProc[ProcNum]->Prob() << "; Rate = " << m_vpProc[ProcNum]->Rate();
		cout << "\nEqm["<<Tree()->StartCalc() << "] : " << m_vpProc[ProcNum]->RootEqm();
		cout << "\nPart of PT(B=0): "; FOR(i,5) { cout << " " << m_vpProc[ProcNum]->GetPT(0)[i]; }
		FOR(i,min(m_pData->m_iSize,5)) { cout << "\n\tSite["<<i<<"]: Raw: " << m_vpProc[ProcNum]->ardL(i).LogP() << " : " << m_vpProc[ProcNum]->L(i) << " = " << m_vpProc[ProcNum]->L(i).LogP(); }
...
void CBaseModel::DoBraOpt(int First, int NTo, int NFr, int Br, bool IsExtBra, double *BestlnL,double tol, bool AllowUpdate)	{
	...
	// Return if branch is not meant to be optimised
	if(!Tree()->pBra(Br)->Opt()) { RETURN_DOBRAOPT; }
...
//	cout << " -> " << *p_x << " = " << *BestlnL;
	// Finish by doing the calculation again to correctly update the partial likelihoods
	DoBralnL(Br,NFr,NTo);
//	cout << " => " << Tree()->B(Br) << " = " << DoBralnL(Br,NFr,NTo);
	if(ori_lnL - 1.0E-6 > *BestlnL)	{
		cout << "\nError: Likelihood appears to have gone down: " << ori_lnL << " -> " << *BestlnL;
		cout << "\nLocal likelihood calc: " << DoBralnL(Br,NFr,NTo);
		cout << "\nFull likelihood calc:  " << lnL(true);
		exit(-1);
	}
